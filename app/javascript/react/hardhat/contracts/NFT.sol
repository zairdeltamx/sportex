...
contract NFT is ERC721URIStorage {
    ...
    function createToken(string memory tokenURI, string memory meta)
        public
        returns (uint256)
    {
        require(
            owner == msg.sender,
            "Only marketplace owner can update Create game NFTs"
        );
    ...
    function updateMarketplace(address newMarketplaceAddress) public payable {
        require(
            owner == msg.sender,
            "Only marketplace owner can update NFT marketplace."
        );
    ...
    function updateMeta(uint256 tokenId, string memory meta) public payable {
        require(
            owner == msg.sender,
            "Only marketplace owner can update NFT metadata."
        );
